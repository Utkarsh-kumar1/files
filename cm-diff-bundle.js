(() => {
    ...
    static fromJSON(json) {
      ...
      for (let i2 = 0; i2 < json.length; i2++) {
        ...
        if (typeof part == "number") {
          ...
          while (inserted2.length < i2)
            inserted2.push(Text.empty);
          inserted2[i2] = Text.of(part.slice(1));
          sections.push(part[0], inserted2[i2].length);
        }
      }
      return new _ChangeSet(sections, inserted2);
    }
    ...
    for (let posA = 0, posB = 0, i2 = 0; i2 < desc.sections.length; ) {
      ...
      if (ins < 0) {
        ...
        for (; ; ) {
          ...
          endB += ins;
          if (ins && inserted2)
            text = text.append(inserted2[i2 - 2 >> 1]);
  ...
  var ChangedRange = class _ChangedRange {
    ...
    static extendWithRanges(diff2, ranges) {
      ...
      for (let dI = 0, rI = 0, off = 0; ; ) {
        let nextD = dI < diff2.length ? diff2[dI].fromB : 1e9;
        ...
        for (; ; ) {
          if (rI < ranges.length && ranges[rI] <= toB) {
            ...
            for (let i2 = dI; i2 < diff2.length && diff2[i2].fromB <= toB; i2++)
              off = diff2[i2].toA - diff2[i2].toB;
            toA = Math.max(toA, end + off);
          } else if (dI < diff2.length && diff2[dI].fromB <= toB) {
            let next = diff2[dI++];
  ...
  function findChangedWrappers(a, b, diff2) {
  ...
  function applyDOMChange(view, domChange) {
    ...
    if (domChange.bounds) {
      ...
      let cmp = state.doc.sliceString(from, to, LineBreakPlaceholder), selEnd, diff2;
      ...
      } else if (diff2 = findDiff(cmp, domChange.text, preferredPos - from, preferredSide)) {
          ...
          if (!update.viewportChanged && this.measureRequests.length == 0) {
            if (this.viewState.editorHeight) {
              if (this.viewState.scrollTarget) {
                ...
                let diff2 = (newAnchorHeight - scrollAnchorHeight) / this.scaleY;
                if ((diff2 > 1 || diff2 < -1) && (scroll == this.scrollDOM || this.hasFocus || Math.max(this.inputState.lastWheelEvent, this.inputState.lastTouchTime) > Date.now() - 100)) {
                  scrollOffset = scrollOffset + diff2;
                  if (scroll)
                    scroll.scrollTop += diff2;
                  else
                    this.win.scrollBy(0, diff2);
                  scrollAnchorHeight = -1;
                  continue;
                }
              }
  ...
  function mergeAdjacent(changes, minGap) {
    for (let i2 = 1; i2 < changes.length; i2++) {
      let prev = changes[i2 - 1], cur2 = changes[i2];
      if (prev.toA > cur2.fromA - minGap && prev.toB > cur2.fromB - minGap) {
        changes[i2 - 1] = new Change(prev.fromA, cur2.toA, prev.fromB, cur2.toB);
        changes.splice(i2--, 1);
      }
    }
  }
      ...
      if (lenA && lenB || lenA > 3 || lenB > 3) {
        ...
        if ((!lenA || !lenB) && lenBefore && lenAfter) {
          ...
          if (changeLen > lenBefore && a.slice(boundBefore, change.fromA) == changeText.slice(changeTo - lenBefore, changeTo)) {
            change = changes[i2] = new Change(boundBefore, boundBefore + lenA, change.fromB - lenBefore, change.toB - lenBefore);
            ...
            change = changes[i2] = new Change(boundAfter - lenA, boundAfter, change.fromB + lenAfter, change.toB + lenAfter);
        ...
        if (lenBefore || lenAfter) {
          change = changes[i2] = new Change(change.fromA - lenBefore, change.toA + lenAfter, change.fromB - lenBefore, change.toB + lenAfter);
        } else if (!lenA) {
          ...
          if (first > -1 && (len = first - change.fromB) <= maxScanAfter && b.slice(change.fromB, first) == b.slice(change.toB, change.toB + len))
            change = changes[i2] = change.offset(len);
  ...
  function asciiWordChar(code2) {
    return code2 > 48 && code2 < 58 || code2 > 64 && code2 < 91 || code2 > 96 && code2 < 123;
  }
  ...
  function findWordBoundaryAfter(s, pos, max) {
    if (pos == s.length || !wordCharBefore(s, pos))
      return pos;
    for (let cur2 = pos, end = pos + max, i2 = 0; i2 < MAX_SCAN; i2++) {
  ...
  function diff(a, b, config2) {
    var _a2;
    let override = config2 === null || config2 === void 0 ? void 0 : config2.override;
    if (override)
      return override(a, b);
  ...
  function toChunks(changes, a, b, offA, offB, precise) {
    ...
    for (let i2 = 0; i2 < changes.length; i2++) {
      ...
      while (i2 < changes.length - 1) {
        let next = changes[i2 + 1];
  ...
  function updateChunks(ranges, chunks, a, b, conf) {
    if (!ranges.length)
      return chunks;
    let result = [];
    for (let i2 = 0, offA = 0, offB = 0, chunkI = 0; ; i2++) {
      let range = i2 == ranges.length ? null : ranges[i2];
  ...
  function mapPos(pos, chunks, isA) {
    ...
    for (let i2 = 0; ; i2++) {
      let next = i2 < chunks.length ? chunks[i2] : null;
    ...
    update(deco, tr) {
      ...
      if (deco.size && tr.state.field(ChunkField) != tr.startState.field(ChunkField, false)) {
        ...
        for (let chunk of tr.state.field(ChunkField)) {
          deco.between(sideA ? chunk.fromA : chunk.fromB, sideA ? chunk.toA : chunk.toB, (from) => {
            touchingChange.push(from);
          });
        }
  ...
  function buildCollapsedRanges(state, margin, minLines) {
    ...
    let prevLine = 1;
    for (let i2 = 0; ; i2++) {
      let chunk = i2 < chunks.length ? chunks[i2] : null;
      let collapseFrom = i2 ? prevLine + margin : 1;
    ...
    */
    ...
    updateRevertButtons() {
      ...
      for (let i2 = 0; i2 < this.chunks.length; i2++) {
        ...
        while (next && +next.dataset.chunk < i2)
          next = rm2(next);
        if (next && next.dataset.chunk == String(i2)) {
          if (next.style.top != top2)
            next.style.top = top2;
          next = next.nextSibling;
        } else {
          dom.insertBefore(this.renderRevertButton(top2, i2), next);
        }
      }
      while (next)
        next = rm2(next);
    ...
    */
    parseDialect(dialect) {
      ...
      for (let i2 = 0; i2 < values.length; i2++)
        if (!flags[i2]) {
          for (let j = this.dialects[values[i2]], id; (id = this.data[j++]) != 65535; )
  ...
  // cm-diff.js
  window.createCodeMirrorDiff = function(containerId, originalCode, modifiedCode) {
